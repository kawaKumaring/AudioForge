...
> - 기준일: 2026-09-08 (직전 2026-09-07)
> - 기준선: master — 정식 **v1.8.0** (2026-09-10 병합: 관리자 코드 검수 4건 + 상태 관리 정리 —
>   목소리 교체 상태를 명시 상태로, 자동 저장 유실·저장 실패 표시, 복원 결과 동일성 확인,
>   대화 분리 페이드 경계, 기본 목소리·감정 슬롯 중복 보고자)
> - 이전 기준선 정식 **v1.7.0** (2026-09-09 병합: 실행 기록을 답할 수 있게 — 난수 씨앗 배선,
>   기록 항목 40→73, 참조 구간·참조 수치·환경, 청취 판정 기록)
> - 이전 기준선 정식 **v1.6.0** (2026-09-08 병합: 목소리 구간 선택 전면 개선 — 낱말 경계로 꽉 찬 음원 개방,
>   구간 길이 기준 교체(밀도 → 담긴 발화 초), 목표 길이 설정, 구간 되돌리기, 대사 편집 중 화면 흔들림 제거,
>   합성 중 설정 변경 오류, 파이썬 검사 27개 부활)
> - 이전 기준선 정식 v1.5.0(2026-09-08) · v1.4.0(2026-09-07) 은 역사 기록이다.
> - 개발선: develop — **v1.9.0-dev**
> - 이전 기준선 `master@46949b7`(v1.3.0) 은 역사 기록이다.
> - 이전 기준선 `master@fa0e907` · `develop@4b64947` 는 역사 기록이다.
>
> 이 문서는 방향, 순서, 완료 조건을 기록한다. 실제 완료 여부는 커밋, 회귀 테스트, 실작업 검증과 사용자 청취 결과로만 확정한다.

## 제품 정체성과 방향 (2026-09-08 확정)

**음원 종합 에디터 툴이다.** 다섯 모드(음악 분리·대화 분리·텍스트 추출·트랙 분할·음성 합성)를
모두 유지하고, 전부 성능과 기능을 올리는 것이 목표다. 음성 합성에 오래 붙어 있는 이유는
**가장 난이도가 높고 아직 제대로 구현되지 않았기 때문**이며, 다른 기능을 접기 위한 것이 아니다.

- **배포**: 상업 배포하지 않는다(개인용). 그래서 기능 선택에서 이용 조건이 걸림돌이 되지 않고,
  쓸 수 있는 기능은 전부 적용한다. `model-licenses.json` 의 `commercial: false` 가 그 선언이다.
  상업 배포로 바뀌면 게이트가 비상업 모델을 막는다(그때 교체 대상: NLLB-600M 등).
- **감정 기능**: **보류.** 지우지 않고 그대로 둔다. 먼저 툴을 정리·최적화해 완전하게 만들고
  안정화된 뒤에 다시 진행한다. 지금 상태의 사실은 남겨 둔다 — 감정·표현 제품 코드 9,329줄에
  실제 적용 경로가 없고(`model_applied` 가 참이 되는 코드 없음, `expressive_planner` 를 합성 경로가
  import 하지 않음), 감정 태그 50종의 효과는 "그 감정용 음원을 등록했으면 그 파일을 참조로 고른다"뿐이다.
- **지금의 우선순위**: 정리 → 최적화 → 안정화. 새 기능보다 이미 있는 것을 제대로 만드는 것이 먼저다.
