...
- **두 경로의 출력이 서로 같았다** — 세그먼트 8, 단어 30, 글자 128, 두 결과의 글자 일치율
  **100%**, 구간 시작·끝 경계 차이 **0.00초**, 언어 ko, 연속 중복 0, 시간 단조 증가.
  ★**이 100% 는 두 경로가 서로 같다는 뜻이지, 정답을 100% 맞혔다는 뜻이 아니다.**
  정답 대본과 맞춰 본 것이 아니므로 이 값으로 정확도를 말할 수 없다.
- 시간(각 실행은 **서로 다른 프로세스**이고, 모델도 매번 새로 적재했다 — 같은 프로세스에서
  모델을 재사용한 수치가 아니다):
  · 기존 경로 = 적재 20.73s + 전사 5.99s (합 27.4s)
  · 새 경로 1회차 = 적재 3.43s + 전사 **11.44s**
  · 새 경로 2회차 = 적재 3.17s + 전사 **1.37s** / 3회차 = 적재 3.21s + 전사 **1.28s**
  ★1회차만 전사가 느린 **원인은 확인하지 않았다.** 디스크에 남는 CUDA 컴파일 결과의 재사용
  같은 것을 **추정**할 수 있을 뿐이고, 측정으로 뒷받침하지 않았다. 추가 측정은 하지 않았다.
