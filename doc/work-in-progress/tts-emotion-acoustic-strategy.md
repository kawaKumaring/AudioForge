...
> ⚠️ **2026-09-05 방향 정정.** 아래 사다리의 기술 판정은 그대로 유효하지만, `reference_matched` 를
> 제품 목표로 삼은 것은 목표 축소였다. 사용자가 요구한 감정 기능은 **음률 규칙을 조사해 새 대사에
> 배치하는 것**이며 감정마다 음원을 등록하는 절차가 아니다. 목표와 규칙 설계는
> [감정 음률 규칙 설계](tts-emotion-rule-design.md)가 소유한다. 이 문서는 capability·프로필·참조
> 선택의 기술 결론을 계속 소유한다.
