...
상태: **연결은 끝났다. 이 장비의 다운로드 인증(토큰)만 기다린다.**
고를 수 있는 경로로 붙였고 **기본은 기존 엔진**이다. master 병합 안 함.

## 지금 막혀 있는 것 — **이 장비의 다운로드 인증** (2026-09-15 확인)

공식 페이지: <https://huggingface.co/pyannote/speaker-diarization-community-1>

**모델 페이지의 이용 조건 수락과 이 장비의 다운로드 인증은 다른 일이다.**
조건은 수락됐다고 들었지만, 그것은 **계정**에 기록되는 일이고 이 컴퓨터에는 그 계정으로
요청하고 있다는 증표(토큰)가 있어야 한다.

이 장비에서 확인한 것:

- 토큰이 **없다** — 환경변수(`HF_TOKEN` / `HUGGING_FACE_HUB_TOKEN` / `HUGGINGFACE_TOKEN`)에도,
  로그인 저장 파일(`C:\Users\<사용자>\.cache\huggingface\token`)에도 없다.
- 그래서 가중치 파일 요청이 여전히 **HTTP 401** 이다. 내려받지 않았다.

### 안전한 로컬 인증 방법 (둘 중 하나만 하면 된다)

어느 쪽이든 **토큰 값을 채팅·로그·Git 에 남기지 않는다.** 토큰 권한은 **읽기(Read)** 면 충분하다.

**방법 1 — 로그인해서 이 컴퓨터에만 저장 (권장)**

    externals\diarization_venv\Scripts\huggingface-cli.exe login

  실행하면 토큰을 물어본다. 붙여 넣으면
  `C:\Users\<사용자>\.cache\huggingface\token` 에 저장된다 — 저장소 안이 아니라
  사용자 폴더이고, Git 에 들어가지 않는다. 한 번만 하면 된다.

**방법 2 — 이번 한 번만 환경변수로**

  PowerShell 창을 하나 열고 그 창에서만:

    $env:HF_TOKEN = "<토큰>"

  그 창을 닫으면 사라진다. 시스템 환경변수에 영구 등록할 필요는 없다.

**하지 말아야 할 것**: 토큰을 파일로 저장소 안에 두거나, 채팅에 붙여 넣거나,
스크립트에 적어 두는 것. 어느 것도 필요 없다.

### 그다음은 자동이다

토큰이 준비되면 가중치를 아래 자리에 받아 두면 된다. 코드 변경은 필요 없다.

    externals/diarization_models/community-1/

그러면 화면의 **대화 > 분석 방식 > Community-1** 이 바로 동작한다.
