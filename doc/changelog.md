...
## 2026-09-08 (4) — 같은 참조를 다시 살펴보고 다시 자르던 반복 제거

사용자가 실행 로그에서 "뭔가 틀고 나서 하는 게 많다"고 지적했다. 로그의 정체와 낭비를 코드로 확인했다.

### 로그에 보이던 파이썬 호출이 무엇인가
- `refanalyze` 참조 살펴보기(길이·표본율·구간 추천·파형). GPU 없음, 수백 ms.
- `reftrim` 고른 구간을 24kHz 모노로 잘라 내고 **그 클립을 whisper 로 전사해 검증**한다(말 도중 절단·
  대사 불일치 차단). 로그의 `Detected language: Korean` 과 프레임 진행 표시가 이것. **건당 2~3초로 가장 비싸다.**
- `qwenpre`(Qwen 준비 상태·모델 판 목록) · `pitchpre`(음높이 후처리 지원) 각 1회.
인물 한 명당 살펴보기 1 + 자르기 1 이 기본이고 기본 목소리도 같은 한 쌍을 쓴다. 여러 명이면 사람 수만큼 곱해진다.

### 고친 낭비
분석·자르기에 결과 저장이 없었다. 메인의 중복 방지는 **동시 호출만 합치는 방식**이라 끝나면 지운다.
그래서 패널이 다시 마운트되기만 해도 같은 파일을 처음부터 다시 분석했다.
- `preview-cache.ts`: (인물|경로|파일 크기·수정 시각|질문) 을 열쇠로 결과를 최대 24개 보관.
  파일 내용이 바뀌면 열쇠가 달라져 저절로 무효화된다. **성공한 결과만** 저장한다 —
  실패를 저장하면 원인이 사라진 뒤에도 같은 실패를 되돌려 준다.
- 자르기는 같은 파일·같은 구간이고 그 클립이 살아 있고 그 인물이 여전히 그 폴더를 소유할 때만 건너뛴다.
  확정에 성공하면 그 인물의 옛 항목을 버린다(폴더가 바뀌었으므로).
- 열쇠 구분자는 공백이 아니다 — 경로에 공백이 흔해 서로 다른 조합이 같은 열쇠가 될 수 있다.
  제어문자도 쓰지 않는다(작성 중 실제로 리터럴 NUL 이 들어가 grep 이 파일을 이진으로 봤다. `KEY_SEP='|#|'`).

### 측정 (실제 앱, 같은 시나리오 전후 비교)
- 첫 측정에서 **호출 수가 전후 동일**했다(살펴보기 4·자르기 3). 그 시나리오의 네 번은 서로 다른
  (인물|파일) 조합이라 **줄일 중복이 애초에 없었다.** 캐시가 한 일이 없다는 뜻이다.
- 그래서 반복을 만들어 다시 쟀다: 준비된 목소리의 설정을 닫았다 다시 열기.
  캐시 없음 → 살펴보기 **4회에서 5회로 증가**. 캐시 있음 → **4회 그대로.**
- 즉 이 최적화의 효과는 "같은 참조를 다시 열 때"에 나타난다. 사람 수만큼 처음 도는 호출은 줄지 않는다.

검증: 캐시 단위 테스트 6건 신설 · `npm run verify:ui` 8/8 통과 · 실제 앱 17/17.

## 2026-09-08 (3) — 인물 목소리 준비를 훅으로 분리 (허브 위험 완화 첫 조각)

배경: 이 로직이 `TTSEditor.tsx`(1,772줄·훅 61개) 안에 다른 여섯 관심사와 섞여 있었다.
오늘 여기서 결함 둘이 나왔고(드라이버가 자기 진행 보고로 자신을 언마운트 / 참조 트림 충돌),
고칠 때마다 그 거대한 파일을 통째로 다시 읽어야 했다.

- `src/renderer/hooks/useSpeakerVoicePrep.tsx` 신설. 소유하는 것:
  자동 준비 대상 선택(한 번에 한 명) · 구간 편집기 만들기 · 목소리 교체 실패 복구와 알림 ·
  목소리 지정 / 다시 준비. 셸에는 **'어디에 그리는가'만** 남는다.
- 셸 1,772줄 → 1,681줄. 준비 내부 상태(autoPrepDone·prevGoodVoice)는 셸에서 사라졌다.
- 남긴 것: 첫 인물이 기본 목소리 준비 결과를 이어받는 흐름. 기본 목소리 상태와 엮여 있어
  이번에 건드리지 않았다(다음 정리 대상). 구간 추천·정책·확정은 여전히 패널과 파이썬의 몫이다.
- 회귀 가드: '준비 규칙은 훅이 소유한다 — 셸에 다시 두지 않는다' 계약 추가.

### 이 과정에서 드러난 검사 취약점 (2순위 문제의 실물 사례)
계약 테스트 4곳이 **옮겨간 함수 이름을 블록 경계로** 쓰고 있어서, 동작이 그대로인데도 깨졌다.
`SHELL.indexOf('const requestSpeakerSource')` 가 -1 이 되어 slice 가 파일 전체를 집고 금지 문자열
검사가 엉뚱하게 실패한 경우까지 있었다. 넷을 소유 모듈(훅)을 보도록 고쳤고, 경계는 셸에 남는
것으로 잡았다. **모양을 검사하면 리팩터링마다 이 비용을 낸다** — 실제 앱 검사(UI 경로 15건)는
같은 변경에서 한 번도 깨지지 않았다.

검증: `npm run verify:ui` 8/8 통과(타입 2종·단위 계약 1,352건·모델 조건·빌드·파이썬 스모크·
모델 판 계약·실제 앱 UI 경로 15건). GPU·합성 없음.

## 2026-09-08 (2) — 총괄 점검에서 나온 구조 개선 4건

코드를 계측해 위험 분포를 본 뒤(파이썬 32,675줄 / 렌더러 12,387줄 / 허브 둘: TTSEditor 1,772줄·훅 61,
tts_worker 3,512줄·함수 88), 결정이 필요 없는 항목부터 실행했다.

- **병합 전 확인을 한 줄로**(`npm run verify`): 타입 2종·단위·계약·모델 조건·빌드·파이썬 스모크·모델 판 계약.
  약 15초, GPU 없음. `verify:ui` 는 실제 앱 UI 경로 15건까지(약 60초, GPU 없음), `verify:app` 은 합성 1회까지(GPU).
  건너뛴 항목은 사유와 함께 요약에 남긴다 — 조용히 통과시키지 않는다.
  배경: 정식 v1.4.0 을 올릴 때 이식성 검사 1건이 실패 상태였고 감정 정의 드리프트 가드는 대상 파일이
  옮겨간 뒤 줄곧 무력했다. 사람이 기억해서 챙기는 방식이 실패했다는 증거다.
- **npm script 미연결 실제 앱 드라이버 4개 연결**: convenience / synth-tabs / work-restore / speaker-rename.
  기억해야만 돌아가는 검사는 돌지 않는다.
- **모델 이용 조건 게이트**(`model-licenses.json` + `scripts/check-model-licenses.mjs`):
  인벤토리를 저장소 안으로 옮겼다. 예전 인벤토리는 `externals/`(.gitignore) 안이라 낡은 채 남아
  **핵심 합성 엔진 Qwen3-TTS 조차 빠져 있었고** 두 항목은 license 가 null 이었다.
  게이트가 즉시 찾은 것: GPT-SoVITS 가중치 누락, htdemucs_ft 누락, Mel-Band 커뮤니티 가중치 조건 미확인,
  번역 기본값 NLLB-600M 이 CC-BY-NC-4.0(비상업)이며 **기본 경로**에 있다는 사실.
  배포 성격을 명시했다(commercial=false). 상업 배포로 바꾸면 게이트가 비상업 모델을 실패시킨다.
- **실험 런타임 13.3GB 삭제**: `_local` 18GB → 4.8GB. seed-vc 는 음질 저하 + GPL-3.0 으로 제품 후보에서
  제외됐고 결론이 문서에 남았으므로 venv·클론·HF 캐시를 지웠다. **판단 근거인 음원 37개(약 15MB)는 그대로 둔다.**
  재현이 필요할 때를 위해 고정 SHA·버전·가중치 목록을 `RUNTIME-REMOVED.md` 에 남겼다.

### 결정 대기 (이 둘이 나머지 일의 크기를 정한다)
- **감정 기능의 위치**: 감정·표현 관련 제품 코드 9,329줄이 있는데 `model_applied`/`post_applied` 가
  참이 되는 코드가 없고, `expressive_planner.py`(1,431줄)를 합성 경로가 import 하지 않는다.
  기본 엔진(Qwen) 경로에서 `emotion_id` 는 오류·진단 객체에만 실린다. 감정 태그 50종의 실제 효과는
  "그 감정용 음원을 등록했으면 그 파일을 참조로 고른다" 하나다. 정식 기능으로 밀지, 축소 선언할지,
  현 상태를 화면에 명시할지 결정이 필요하다.
- **제품 정체성**: 5개 모드를 유지하지만 투자는 음성 합성에 집중돼 있고 나머지 넷은 자동 검사가 얇다.

## 2026-09-08 — 편의성 수정 6건 (여러 명 인물 추가·자동 준비·구간 편집·고급 설정)

사용자 실사용 지적을 그대로 받아 고쳤다. 실제 앱을 격리 환경으로 띄워 9건 확인
(`test/e2e/tts-convenience-dev.e2e.mjs`, 전부 통과). GPU·음성 생성 없음.

- **자동 준비가 스스로 멈추던 결함**(가장 큰 것): 인물에 목소리를 지정하면 카드가 '목소리 확인 중'에서
  멈추고 사용자가 구간 편집기를 열어 손으로 확정해야 했다. 원인 둘을 각각 고쳤다.
  · 보이지 않는 준비 드라이버가 다음 대상을 **준비 상태 문구가 비었는지**로 골랐다. 구간 패널이 분석을
    시작하며 진행 문구를 그 자리에 올리는 순간 조건이 깨져 드라이버가 자기 자신을 언마운트했다.
    → 패널이 `onAutoConfirmSettled`(끝났다는 신호)를 내고, 드라이버는 그 신호나 파일 교체 전까지 놓지 않는다.
  · 참조 트림 IPC 가 배타 가드라 겹친 두 번째 호출을 곧바로 거절했다. 기본 목소리 준비와 인물 자동 준비가
    동시에 구간을 확정하려 하면 뒤에 온 쪽이 실패했다. → `createSerialLane` 으로 거절 대신 줄 세우기.
    파이썬을 동시에 두드리지 않는다는 목적은 유지되고, '트림 중 합성 차단' 계약도 그대로다.
- **이름 없이도 목소리 지정**: 새 인물 추가에서 이름을 비워도 목소리 지정·완료가 열린다. 자동 이름은
  `인물A`·`인물B`…(id 기준으로 이미 쓰는 것은 건너뛴다). 고른 파일 이름을 창에 보여 준다 — 그걸 보고
  이름을 정하는 경우가 있기 때문이다. 목소리를 지정한 뒤에는 그때 쓴 이름을 잠그고, 이름 변경은
  카드의 '이름 바꾸기'(목소리·구간·감정 설정이 함께 따라가는 기존 경로)로 안내한다.
- **구간을 맘대로**: 파형을 끌어 구간을 잡는다(누른 곳~뗀 곳). 시작·길이를 0.01초 단위 숫자로 직접
  넣는다. 시작 슬라이더 상한이 '남은 길이'여서 뒤쪽을 보려면 길이를 먼저 줄여야 했던 것을 고쳤다 —
  원본 끝을 넘으면 시작을 되돌리지 않고 길이를 줄인다. 엔진 정책 범위는 그대로 지킨다.
- **음성 모델 판 고르기**(엔진·진단): 설치된 판만 목록에 뜬다. 이 기계 실측 0.6B·1.7B 두 판.
  찾는 곳은 저장소 안 두 곳뿐이고 새로 내려받지 않는다. 알 수 없는 id·목소리 복제 미지원 판은
  거부한다(조용히 다른 모델로 내려가지 않는다). 결과 metadata 에 `qwen_model_variant` 로 남는다.
- **음성 탭 정리**: 실제 조작(참조 방식)을 맨 위로, 보조 항목 셋(감정별 목소리·목소리 보관함·목소리 구성
  저장)을 접었다. 긴 설명은 '설정 설명 표시'를 켰을 때만. 참조 전사는 오류 카드가 직접 여는 자리라 접지 않았다.
- 부수: 감정 정의 드리프트 검사가 옮겨간 파일(`lib/emotions.ts`)을 못 보고 늘 실패하던 것 복구(스모크 3/3),
  `test_reference_region_policy.py` 의 sys.path 제한 보정(14건 통과), 09-07 대조군 드라이버의 절대 경로 제거,
  `ReferenceRegionPanel.tsx` 의 리터럴 NUL 바이트 제거(grep 이 이진으로 보던 문제).

남은 것: 모델 판 고르기는 Qwen3 경로만이다(다른 엔진은 설치본이 하나뿐이라 고를 것이 없다).
실제 합성 품질 확인은 사용자 청취로 남는다.

## 2026-09-07 — 정식 v1.4.0 (master 병합)

9/5 23:44 UX 마감 상태(`39fb021`)를 그대로 정식으로 올린다. 그 이후 develop 커밋은 감정·변환 조사 문서와 `tools/`·`test/` 조사 도구만이며 제품 코드 변경은 없다.

- **포함**: 여러 명 대화 UX(전체 폭 탭·카드·대화 추가·인물 이름 변경 연동), 목소리 설정 저장·자동 복원, 새 인물 목소리 자동 준비,
  엔진별 참조 길이 정책(Qwen3 정책 신설), 장문 안전 분할·종료 원인 분류, 참조 상태 분리, 참조 예산 실측화.
- **실사용 확인**: 사용자가 앱에서 직접 생성한 결과가 음질 문제 없이 나오는 것을 확인(2026-09-07). 이틀간의 잡음·떨림 사건은
  전부 제품 밖 실험 경로(seed-vc 변환)였고, 제품 경로 코드 검수에서 파형을 변질시키는 단계는 없었다(work-in-progress/tts-emotion-rule-design.md §6.19e~g).
- **.gitignore**: `/resources/`·`/사용자/` 추가 — 사용자 목소리·감정 자료가 저장소에 들어가지 않게 한다.
- **알려진 남은 위험**(제품 안, 미착수): 참조 억양 반영 방식의 벤더 비례 하드 컷(앞머리 잔류 가능), 발행 게이트의 최대치 샘플 거부(확률적 실패 후보),
  참조 자동 구간의 대표성 기준 부재, 문장 간 목소리 흔들림(온도·seed 미노출).

## 2026-09-05 — 합성 화면 재정리: `한 명 | 여러 명` 은 화면 전체를 전환하는 전체 폭 탭

상세: doc/script-scene-architecture.md §19.

- **탭 위치**: 합성 메뉴 바로 아래 전체 폭(분할 화면의 탭과 같은 위치·계층). 대사 옆의 작은 탭 제거.
- **한 명**: 1 목소리 → 2 대사 한 칸 → 3 말하는 느낌(기존 흐름). **여러 명**: 단일용 목소리 영역 없음(기본 목소리 준비만 숨겨서 실행),
  1 인물과 대사(카드) → 공통 생성 옵션(한 번) → 2 말하는 느낌. 상단 공용 감정 버튼 제거(카드의 `+ 감정`만).
- **카드**: 인물·짧은 목소리 상태·대사 한 칸·`+ 감정`·이동·삭제. 목소리 상세(구간 수정 포함)는 누른 카드 안에 하나만.
- **대화 추가**: `+ 대화 추가` 하나 → 설정 창(기존 인물/새 인물) → 카드 생성·포커스. 대사는 카드에서. 취소 = 무변경.
  시작 카드·카드별 추가 버튼·목록 아래 인물 select+대사 입력 행 제거. 시작 카드 이름 변경 시 슬롯이 따라간다.
- **첫 인물 연결 결함 수정**(준비됨 vs 목소리 확인 중 불일치): (1) 초기 binding 이 기본 목소리의 확정 클립을 이어받지 않던 것 →
  확정 클립을 인물 key 로 복사(`audio:adopt-reference-clip`)해 원본·구간·유효 참조를 옮기고, 준비 도중 전환도 완료 시 갱신.
  사용자가 목소리를 바꾸면 이어받기 종료. (2) 참조 구간 편집기가 심은 슬라이더 값(사용 중/추천 구간)을 '구간 변경' 으로 보고
  준비를 내리던 것 → 심은 값이면 무변경. 같은 음성·구간의 재등록·재확정 요구 없음.
- **인물 이름 변경**(카드 머리 '이름 바꾸기'): 현재 작업의 모든 발화 표기·목소리 슬롯(확정 구간)·감정별 설정이 새 이름으로 함께 유지. 저장된 목소리 구성은 건드리지 않고 별칭으로 읽는다(정체성·자산 연결 유지). 충돌 시 거부(병합 없음). 원문 직접 편집과 구분.
- **UX 마감 2차**: 낡은 분석 문구("모든 화자가 기본 참조 / 다음 단계") 제거·실제 판정 파생, 카드에 상태 텍스트 + '목소리 설정' 버튼(구간 선택 필요면 편집기 바로 열림), 첫 대사 반영 시 카드 점프 제거(유령 카드·포커스 유지), '+ 감정·쉼' 메뉴(쉼 0.3/0.5/1.0초), '기본 인물' 은 기존 대본 호환 상태에서만 노출. 첫 인물 참조의 실제 파일 복사 방식과 수명·복원 한계를 §19 에 명시.
- **쉼 표기 카드 결함 수정**: 줄 끝 `[쉼 N]` 이 계획 구간 밖이라 카드가 전부 사라지던 것 → 같은 줄의 쉼은 그 행에 흡수(쉼 전용 줄은 제외). §19b.
- 검증: tsc web/node 0 오류, 관련 계약 테스트 재작성·통과(MultiSpeakerDialogue·InitialBinding 등 107건), 개발 앱 격리 userData
  1회 19/19(`test/e2e/tts-synth-tabs-dev.e2e.mjs`, 캡처 4장). GPU·음성 생성·전체 회귀·build 없음.
