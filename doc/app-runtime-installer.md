...
## 지문이 어긋났을 때 — **늘어난 것과 사라진 것은 다른 일이다** (2026-09-25)

### 실제로 일어난 일

잘 쓰던 앱이 갑자기 "환경 점검: 미비" 를 띄우고 **4GiB 재설치**를 요구했다.
실제 원인은 9월 24일 일본어 지원을 위해 패키지 **3개가 정당하게 추가**된 것이었고,
기록만 갱신하면 30초에 끝나는 상황이었다.

화면은 두 가지를 잘못했다.

1. 개수가 114 → **117 로 늘었는데** "패키지가 삭제·변경되었습니다" 라고 말했다.
2. 보이는 선택지가 **전체 재설치뿐**이었다. `verify --relink` 라는 싼 길이
   도구에 이미 있었는데 화면에 없었다 — **보이지 않으면 없는 것과 같다.**

사용자는 "손상됐구나" 로 읽고 내려받기를 시작했다.

### 고침

- 지문에 **배포 이름 목록**을 함께 기록한다. 예전에는 개수만 남겨서 어긋났을 때
  "무엇이" 바뀌었는지 말할 수 없었다. 이름은 비민감하다(패키지 이름·판 번호뿐).
- 어긋나면 **늘어난 것과 사라진 것을 갈라** 이름까지 말한다.
- 사라진 것이 없으면 `repairable_by_relink` 가 참이 되고, 화면이 **재설치 전에**
  `verify --relink` 를 먼저 권한다.
- 옛 기록(이름 없음)에는 방향만 말하고 **모르는 것을 아는 척하지 않는다** —
  "늘었습니다 — 사라진 것이 있는지는 이 기록으로 알 수 없습니다".

### 바뀐 화면

```
사유      : FINGERPRINT_MISMATCH — 설치된 패키지가 기록과 다릅니다.
            기록 114개 → 현재 117개. 늘어난 것 3개: pyopenjtalk_plus 0.4.1.post9,
            sudachidict_core 20260723.1, sudachipy 0.7.0.
            사라진 것은 없습니다 — 손상이 아닐 수 있습니다.

            사라진 패키지가 없습니다 — 다시 설치하지 않아도 될 수 있습니다.
            먼저 이것을 해 보세요(수십 초, 내려받기 없음):
                python app_env_installer.py verify --relink
            실제로 열리는지 확인한 뒤 기록만 갱신합니다. 실패하면 그때 설치하세요.
```

### 남은 한계

지문은 여전히 **dist-info 목록**만 본다. 패키지 본문이 손상된 경우는 못 잡는다 —
그것은 `verify` 의 실제 import·모델 로딩이 잡는다. 두 검사의 범위를 섞지 말 것.
