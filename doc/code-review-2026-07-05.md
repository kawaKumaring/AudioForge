...
| 9-2 번역 개선 | ⚠️ NLLB 1.3B 옵션 추가했으나 효과 없음 확인 → 진짜 레버는 LLM(외부 서비스 결정 대기) |
...
**남은 사용자 결정 대기**: LLM 번역(9-2 후속, API/로컬 결정), pyannote(9-6, HF 토큰),
GPT-SoVITS 일본어/파인튜닝(VS Build Tools + 데이터).
...
- **진짜 레버 = LLM 번역**: 문맥 인지 + 환각 억제로 구어체 JA→KO에서 NLLB 대비 격차 큼.
  단, 외부 API 키 또는 로컬 LLM 결정 필요(환경 리스크·비용) → 사용자 결정 대기.
