...
> **상태 (2026-09-03)** — 2026-08-23 조사 기록이며 그대로 보존한다. 이 문서의 모델 비교와
> PoC 후보 판단은 **현재도 유효한 근거**다. 특히 CosyVoice 3(한국어·제로샷 복제·instruction
> 동시)와 IndexTTS-2.5(감정 제어 구조는 강하나 **한국어 미지원**)의 판단은 v1.4 의 적용 통로
> 네 갈래 중 "별도 엔진 격리 PoC"의 근거로 그대로 쓴다.
>
> 그 뒤 바뀐 사실 하나: **Qwen 1.7B Base 가 로컬에 설치됐다.** Base 이므로 §3.1 의 결론
> (Base 에는 instruction 감정 제어가 없다)이 그대로 적용된다 — 크기가 커졌다고 감정 제어가
> 생기지 않는다. 현재 capability 표는 [감정 음향 전략](../work-in-progress/tts-emotion-acoustic-strategy.md) §7 에 있다.
