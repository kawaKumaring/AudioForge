...
## 19. 합성 화면 재정리 — `한 명 | 여러 명` 은 화면 전체를 전환하는 탭 (2026-09-05)

### 완료 기준(관리자)
합성 메뉴 바로 아래 전체 폭 탭. 사용자는 한 명 또는 여러 명을 고르고, 선택에 맞는 화면에서 **중복 설정 없이** 작업을 끝낸다.
"기존 대사 편집기에 여러 명 기능을 더했다" 는 완료가 아니다.

### 화면 구조
- 합성 → `한 명 | 여러 명`(전체 폭, `DialogueTabs`, 분할 화면의 탭과 같은 위치·계층. 대사 옆 작은 탭은 제거) → 화면.
- 한 명: 1 목소리(기본 목소리·구간·참조 방식) → 2 대사 한 칸(감정 태그 팔레트) → 3 말하는 느낌. 기존 흐름 그대로.
- 여러 명: 단일용 `1 목소리` 영역을 그리지 않는다(기본 목소리의 분석·추천 구간 자동 확정은 `default-voice-driver` 로 숨겨서
  계속 돈다 — 첫 인물이 이어받는 원천). 1 인물과 대사(시간순 카드) → 공통 생성 옵션(참조 방식, 한 번) → 2 말하는 느낌.
  단일 번호 체계를 끌고 오지 않는다(`flowNumber`).
- 카드 = 인물 · 짧은 목소리 상태(`목소리 선택 필요` / `구간 선택 필요` / `목소리 확인 중` / `준비됨 · N초부터 M초`) · 대사 한 칸 ·
  `+ 감정`(커서 위치) · 위/아래/삭제. 목소리 상세는 **누른 카드 안**에 하나만 펼치고(원본 파형·구간 수정은 `구간 수정` 으로
  필요할 때만), "이 인물의 다른 대사에도 같은 목소리·구간이 적용됩니다" 를 짧게 표시한다. 상단 별도 참조 패널 없음.
- 대화 추가 = 목록 아래 `+ 대화 추가` 하나 → 설정 창(기존 인물 = 준비된 목소리 재사용 / 새 인물 = 이름·목소리) → 완료 시 카드
  생성 + 대사 칸 포커스. 대사는 설정 창에서 쓰지 않는다. 취소 = 원문·인물·목소리 무변경(창에서 한 목소리 지정만 되돌림).
  시작 카드 추가 버튼·카드별 추가 버튼·목록 아래 인물 select+대사 입력 행은 제거. 빈 카드·시작 카드는 blur/Ctrl+Enter 로
  원문에 반영(source patcher 재사용, 빈 카드·설정 창은 원문을 쓰지 않는다).
- 빈 대본 첫 진입: 처음 불러온 목소리를 이어받은 `인물1` 카드 하나. 이름을 바꾸면 슬롯이 새 id 로 따라간다(`moveSpeakerRef`,
  main `audio:rename-reference-clip`).
- 여러 명 상단의 공용 감정 버튼 나열 제거(한 명 전용). 고급 설정의 인물별 목소리 편집기는 없음(안내만), 목소리 구성 저장/불러오기는
  접힌 재사용 기능으로 유지.

### 준비됨 / 목소리 없음(확인 중) 불일치의 실제 원인 (첨부 화면 = develop 최신 빌드였다)
1. 초기 binding 이 기본 목소리의 **확정 클립을 공유하지 않기로** 하고 `clip:'' ready:false('구간 선택 필요')` 슬롯을 만들었다.
   10초 초과 원본(기본 목소리가 구간 클립을 쓰는 보통의 경우)에서 상단은 '준비됨', 첫 인물은 '목소리 확인 중' 이 됐다.
   준비 도중 전환하면 완료 뒤 갱신도 없었고, 같은 파일을 다시 지정하면 새 슬롯이라 구간 확정을 또 요구했다.
2. 참조 구간 편집기(`ReferenceRegionPanel`)가 분석을 끝내고 **사용 중 구간(또는 추천 구간)을 슬라이더에 심는 순간**, start/dur
   effect 가 그것을 사용자의 변경으로 보고 "구간을 변경했습니다 — 다시 확정" 로 준비를 내렸다. 10초 초과 원본에서만 드러나는데
   예전 화면 확인은 8초 파일만 썼다. 카드 패널을 열거나 탭을 왕복할 때 첫 인물·기본 목소리가 '확인 중' 으로 떨어지는 두 번째 원인.

### 교정
- 이어받기(`ttsSpeakerInherit`): 첫 인물(계획 1번 또는 시작 카드 인물1)에 슬롯 생성 + 플래그. 동기화 effect 가 기본 목소리 준비 완료 시
  원본 전체면 그대로, 확정 클립이면 main `audio:adopt-reference-clip` 로 **복사(소유권 분리)** 해 옮기고 준비됨. 준비 중이면 같은
  사유, 완료 시 갱신. 사용자가 그 인물의 목소리를 바꾸거나 해제하면 플래그가 지워져 초기화가 덮어쓰지 않는다. 이후 한 명/여러 명
  목소리는 독립. 2번 이후 인물은 자동 연결 없음. 새 파일은 새 작업(슬롯 비움).
- 편집기: 심은 슬라이더 값과 같으면 아무것도 바꾸지 않는다(`seededRegion`). 사용 중 확정이 있으면 자동 확정을 다시 돌지 않는다.
- 실제 생성 계약과의 대응: 카드 준비됨 = store 슬롯 ready·clip(복사된 클립) = ProcessButton 전송(`slot.clip || slot.source`)
  = Python 라우팅. 화면만 바꾼 것이 아니다.

### 실제 화면 확인 (개발 앱, 격리 userData 1회, `test/e2e/tts-synth-tabs-dev.e2e.mjs`, 18초 실제 발화 fixture)
19/19 통과: 전체 폭 탭·중복 없음 / 한 명 화면 / 여러 명 화면(단일 목소리 영역 0·감정 버튼 0·대화 추가 1) / 준비 완료 후 전환 →
인물1 준비됨(복사된 클립·같은 구간) / 카드 상세 열어도·구간 편집기 펼쳐도 준비 유지(재확정 요구 없음) / 시작 카드 첫 대사 반영 /
대화 추가(기존·새) / 취소 무변경 / 같은 인물 카드 공유 / 탭 10회 왕복 무손실 / 1100·720·560 폭 겹침·넘침·쪼개진 버튼 0 /
준비 도중 전환 → 완료 시 이어받음 / renderer 오류 0. 캡처 4장: `_local/artifacts/diagnostics/ux-redesign-20260905/shots/`.

### 남은 결함·한계
- (해결 2026-09-05) 카드 상세 '이름 바꾸기': 그 인물의 모든 발화 표기(patcher renameSpeaker) + 목소리 슬롯(확정 구간·클립 key) + 감정별 설정·후보 선택 + 목소리 구성(모든 배역)을 새 id 로 함께 옮긴다. 다른 기존 인물과 충돌하면 병합하지 않고 거부 안내. 고급 원문 편집으로 표기를 바꾸는 것은 예전처럼 알림+되돌리기만(슬롯 이동 없음). 원본 전체를 그대로 쓰는 준비 상태(whole)도 '사용 중' 으로 취급해 재마운트가 준비를 내리지 않는다.
- 인물 카드 패널(기본 참조 아님)의 자동 확정은 없다 — 새 인물의 긴 원본은 사용자가 추천 구간을 확정한다.
- 한 명 ↔ 여러 명 전환 때 기본 목소리 편집기가 다시 마운트돼 분석 IPC 가 1회 돈다(준비·확정 상태는 유지).

### UX 마감 (2026-09-05, 2차)
- **이름 변경의 범위**: 카드 머리(이름 옆) '이름 바꾸기' 는 **현재 작업**의 것만 바꾼다 — 그 인물의 모든 발화 표기(patcher
  `renameSpeaker`), 목소리 슬롯(준비·확정 구간·클립 key 이동), 감정별 사용 설정·후보 선택(`인물␟감정` 키 재지정).
  **저장된 목소리 구성은 건드리지 않는다**(사용자가 명시적으로 저장/덮어쓸 때만 갱신). 대신 현재 작업이 별칭
  (`ttsSpeakerRenames`: 구성 안의 인물 id → 지금 id)을 기억해 구성을 **읽을 때만** 옮겨 준다(`applySpeakerRenames`,
  `castSpeakerIdOf`) — 이름이 달라져도 같은 인물의 정체성·자산 연결이 유지된다. 다른 구성의 같은 이름 인물을 같은 사람으로
  간주하지 않는다. 다른 기존 인물과 충돌하면 병합하지 않고 거부 안내. 처음 구현했던 '모든 배역 순회 변경'(renameSpeakerInCasts)
  은 범위 초과라 제거했다. 고급 원문 편집으로 표기를 직접 바꾸는 것은 알림+되돌리기만(슬롯 이동 없음).
- **분석 문구**: "모든 화자가 지금은 기본 참조를 씁니다 / 다음 단계" 제거. 인물 목록 아래 문구는 생성 방식에서 파생하고
  (`speakerReferenceNote`), 여러 명에서는 인물별 상태를 카드와 **같은 판정 함수**(`voiceStatusShort(speakerVoiceOf(id))`)로 보인다.
- **목소리 설정 입구**: 카드 머리에 상태는 텍스트로, 별도 `목소리 설정` 버튼. `구간 선택 필요` 상태에서 누르면 그 카드의 구간
  편집기가 바로 펼쳐진다(`initialRegionOpen`).
- **카드 점프 제거**: 시작 카드·빈 카드의 첫 대사를 반영하면 계획이 새 행을 만들 때까지 같은 자리에 유령 카드(내용 보존)를 두고,
  새 행이 오면 그 대사 칸으로 포커스를 옮긴다. 사용자가 순서를 바꾸지 않는 한 위치가 유지된다.
- **감정·쉼**: 카드의 `+ 감정·쉼` 메뉴에 쉼 0.3/0.5/1.0초 — 기존 문법 `[쉼 0.3]` 을 같은 커서 삽입으로 넣는다. 새 편집기·표현 구조 없음.
- **'기본 인물'**: 화자 표기 없는 대사(기존 대본 호환용 내부 상태)이다. 지금 그 상태인 대사의 선택 상자에서만 보이고, 새 대화의
  인물 선택(대화 추가 설정 창)에는 노출하지 않는다. 기존 연결은 보존.

### 첫 인물 참조의 '실제 파일 복사' — 원래 요청과의 차이와 한계(문서화, 재설계는 별도)
- 관리자가 처음 요청한 것은 **같은 자산을 가리키는 독립 연결**(자산 공유)이었다. 현재 구현은 main 이 기본 목소리의 확정 클립
  폴더를 새 임시 폴더로 **실제 복사**해 인물 key 소유로 등록한다(`audio:adopt-reference-clip`). 같은 경로를 두 key 가 공유하지
  않으므로 한 명에서 참조를 바꾸거나 해제해도 여러 명의 기존 참조는 삭제되지 않는다(이 요건은 충족).
- 한계: 복사본은 임시 폴더(tmp)에 살고 앱 세션 동안만 유효하다. 앱을 다시 켜면 세션 복원이 인물 슬롯을 원본 경로 + '목소리 다시
  확인 필요' 로 재구성하므로(구간·클립은 저장되지 않음) 인물 카드에서 구간을 다시 확정해야 한다. 원본이 옮겨지거나 지워지면
  복원할 수 없다. 클립 두 벌이 디스크를 차지한다(수백 KB). 저장소 구조(내용 기반 자산 저장·참조 카운트)로 바꾸는 것은 후속.

### 쉼 표기와 카드 행 — 마감 화면 확인에서 잡은 결함(2026-09-05)
- 현상: 카드의 `+ 감정·쉼` 으로 대사 줄 끝에 `[쉼 0.3]` 을 넣으면 카드가 전부 사라지고 원문 편집기만 남았다(탭 왕복 확인 12번에서 발견).
- 원인(실측, `python/script_plan.build_structure`): 계획의 발화 구간은 **쉼 표기를 포함하지 않는다**(감정 태그는 포함). 줄 끝의 쉼은
  어느 구간에도 속하지 않아 `NON_WHITESPACE_OUTSIDE` 가 걸리고 구조 편집이 잠겼다. 줄 가운데 쉼은 같은 줄 조각 잇기로 이미 안에 있었다.
- 조치(`groupUtteranceRows`): 발화와 **같은 줄**의 쉼(앞·끝)은 그 행의 조각에 넣는다 — 카드 본문은 "화자 표기를 뺀 나머지 글자 그대로"
  이므로 쉼 표기가 대사 칸에 보이고 그대로 편집된다. 쉼 전용 줄은 흡수하지 않는다(원문 편집기 몫, 정직하게 잠김).
  검사 `src/shared/dialogueSourcePatcher.pause.test.ts`(실측 좌표 5건).

### 후속 목록(이번 마감 범위 밖)
- 새 인물의 긴 음원 자동 구간 확정(지금은 기본 참조만 자동) · 탭 전환 시 기본 목소리 편집기 재마운트로 분석 1회 추가 ·
  고정 합성 버튼 · 인물별 색상 · 생성 옵션 재배치 · 참조 자산 저장소 재설계(공유 방식·복원).

## §20 목소리 설정의 저장·복원 (2026-09-05)

### 감사 — 왜 재시작하면 사라졌는가
- `session.json` 은 **실행 결과 처리 안에서만** 쓰인다(`runner.on('result')`). 합성하지 않으면 파일 자체가
  생기지 않는다. 그래서 "구간 필드를 추가한다" 만으로는 요구를 충족하지 못한다 — 저장 시점이 없다.
- 저장되던 것: 인물별 **원본 경로**·표시 이름·생성 방식. 저장되지 않던 것: **확정 구간**(합성 config 타입에
  `ttsSpeakerRefRegions` 필드가 아예 없었다), 파생 클립(임시), 감정별 참조·후보 선택(복원 시 비움).
- 인물 필드는 세션 복원에서 타입 선언 없이 우회 캐스팅(`oRaw`)으로 읽혀, 구간 누락이 조용히 지나갔다.
- 대조: **감정 참조는 이미 원본+구간을 저장하고** 원본 생존을 실제로 확인해 세 갈래로 복원한다
  (없음→재지정 / 있음+파생클립 사용→구간 재확정 / 있음+원본 직접 사용→준비됨). 인물 쪽에만 없었다.
- `computeRefLiveness` 는 세션 원본과 감정 원본만 `existsSync` 한다. 인물 원본은 확인 대상이 아니었다.

### 재사용할 수 있는 기존 저장소(새로 만들지 않는다)
- **참조 라이브러리**(`shared/referenceLibrary` + `main/services/reference-store`): 앱 소유 userData 안에
  확정 클립을 복사 보관. 동일성 권위가 **내용 해시**이고 manifest 에 경로를 담지 않는다 → 외부 원본이
  옮겨져도 보관 클립은 그대로 쓴다. `resolveClipFile` 이 저장 클립 해시를 대조한다(존재만이 아니라 내용까지).
  이미 배선돼 있으나 **선택이 하나뿐인 모델**이라 인물별 지도가 따로 필요하다.
- **전역 자산 등록부**(`referenceAssets`): 같은 파일의 같은 PCM 구간 = 같은 `assetId`, `sourcePath` ·
  `sourceSha256` · `region` 보유, `assetRefCountAcrossCasts` 로 참조 계수까지 있다. 단 **저장된 구성 안에서만**
  세므로 현재 작업·진행 중 합성의 사용 관계를 모른다 — 이것만으로 삭제 안전을 선언하면 안 된다.
- **전사 필수는 저장 형식이 아니라 모델 요구다.** 저장소는 `transcript: null` 로도 승격하고 상태를
  `TRANSCRIPT_MISSING` 으로 보고한다(sidecar 를 안 만들 뿐). 필수로 막는 것은 화면 쪽 호출부다.
  → 보관 성공과 ICL 사용 가능은 **서로 다른 상태**로 나눌 수 있다.

### 작업 식별자 — 기존 것이 없어 최소로 정한 것
이 앱에는 작업 식별자가 없고, 작업은 **불러온 원본 파일**에 묶여 있다(`setFile` 이 인물 상태를 전부 비운다).
그래서 원본 경로를 정규화한 값(`workKeyOf`)을 열쇠로 쓴다. 전역 화자 이름으로 묶지 않는다 — 다른 작업의
같은 이름은 같은 사람이 아니다. 원본 이동에 대비해 기록에 `sourceSha256` 자리를 두되, **여기서 큰 음원을
해시하지 않는다**(보관 과정에서 알게 된 값만 받아 적는다).

### 개수는 삭제의 이유가 아니다
처음 판은 기록이 20개를 넘으면 오래된 것부터 지웠다. 그런데 **합성한 적이 없는 작업은 대사와 인물 구성이
이 기록에만 있다** — 지우는 것이 곧 사용자의 작업 유실이었다(관리자 정정, 2026-09-05). 지금은 개수를
이유로 어떤 기록도 지우지 않는다. `WORK_DRAFT_RECENT_LIST_LIMIT` 은 **목록 표시 상한**일 뿐이고
(`recentWorkDraftKeys`), 목록에 안 보여도 기록은 그대로 남아 다시 열면 복원된다. 저장 관리 화면·정리
기능은 만들지 않았다. 대가는 설정 파일이 작업 수만큼 커지는 것인데, 한 기록은 경로·대사·인물 몇 줄이라
유실보다 이쪽이 낫다.

### 저장 경계
`workDrafts` 키 하나에만 쓴다. main 의 `settings:set` 이 키 하나만 원자적으로 교체하므로, 자동 저장이
사용자가 명시적으로 저장한 목소리 구성(`voiceCasts`)과 전역 자산 등록부(`referenceAssets`)를 건드리지 않는다.
기록에 `clip` 과 `ready` 는 담지 않는다 — 이번 실행에서만 뜻이 있는 값이라 담으면 재시작 후 "준비됨" 이라
적힌 채 쓸 수 없는 상태가 만들어진다.

### 복원 규칙(`planWorkRestore`)
보관 클립이 성하면 재사용 → 없으면 **저장된 구간 그대로** 재생성 → 둘 다 안 되면 **그 인물만** 재연결 안내.
구간 추천을 다시 돌리지 않고, 다른 구간·다른 인물 목소리로 대체하지 않는다. 계획 단계에서 `ready` 를 올리지
않는다 — 실제 준비가 끝난 인물만 준비됨이 된다. 복원 중 표시는 '목소리 준비 중'이고, 카드가 이 문구를
'구간 선택 필요' 로 뭉개지 않는다(`RESTORE_STATUS`).

### 한 명 ↔ 여러 명 격리(확인한 사실)
파생 클립은 키별 소유다. 키 없는 `releaseReferenceClip()` 은 **`setFile` 과 `reset`**(= 작업 자체가 사라지는
자리)에서만 불린다. 한 명 경로의 참조 변경·해제는 `default` 키만 놓으므로 인물 클립을 건드리지 않는다.
