...
# ★표기를 눈에 보이게 적는다(2026-09-24).
#   예전 표기 '\/...' 는 파이썬이 모르는 이스케이프라 역슬래시가 그대로 남았는데,
#   **같은 글자를 적은 화면 쪽(JS)은 '/' 한 글자로 읽어** 역슬래시가 빠졌다.
#   동작은 이쪽이 맞다(역슬래시는 윈도 파일 이름에 못 쓴다). 다만 그 표기는
#   파이썬이 **앞으로 오류로 바꾸겠다고 예고**한 것이라(SyntaxWarning) 명시 표기로 옮긴다.
