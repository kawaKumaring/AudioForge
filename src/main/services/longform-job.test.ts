...
// ═══════════════════════════════════════════════════════════════════════════
// G. 문구 의존의 fail-safe 성질 — bridge 문구가 바뀌어도 조용히 오작동하지 않는다
// ═══════════════════════════════════════════════════════════════════════════
//
// parseChunkPosition 은 python/qwen_bridge.py 의 한국어 진행 문구를 읽는다. 구조화 필드로
// 바꾸는 것은 bridge/runner 계약 변경이라 지금 범위 밖이다. 대신 '문구가 바뀌면 어떻게 되는가'
// 를 고정한다: 위치를 **모름**으로 떨어뜨려 liveness 로 취급되고, 감시는 느슨해지는 게 아니라
// **더 보수적**으로 동작한다(전진 없음 → stall 축이 잡는다). 조용한 오작동이 없다는 뜻이다.

// 브리지 문구가 바뀌었을 때 나올 법한 형태들. 어느 것도 '전진'으로 오인되면 안 된다.
const CHANGED_FORMATS = [
  'Synthesizing... (sentence 3/12, chunk 1/2 done)',      // 영문화
  '합성 중... (문장 3/12 · 조각 1/2 완료)',                  // 구분자 , → ·
  '합성 중... (3/12 문장, 1/2 조각 완료)',                   // 어순 변경
  '합성 중... (문장 3/12, 조각 1/2 끝)',                     // 완료 → 끝
  '합성 중... (문장 3/12)',                                  // 조각 정보 제거
  '합성 중... 62%',                                          // 수치만
  '{"seg":3,"chunk":1,"phase":"complete"}'                   // 구조화로 바뀜(문구 아님)
]

test('[fail-safe] 문구가 바뀌면 위치는 "모름"이 된다 — 절대 잘못된 위치를 지어내지 않는다', () => {
  for (const m of CHANGED_FORMATS) {
    assert.equal(parseChunkPosition(m), null, `"${m}" 를 위치로 해석하면 안 된다`)
  }
})

test('[fail-safe] 문구가 바뀌면 전진이 아니라 liveness 로 판정된다(예산이 조용히 연장되지 않는다)', () => {
  for (const m of CHANGED_FORMATS) {
    const clock = makeClock()
    const wd = createJobWatchdog({ now: clock.now })
    assert.equal(wd.observe({ percent: 62, message: m }), 'liveness', `"${m}"`)
    assert.equal(wd.ledger.completedCount, 0, '해석 못 한 문구는 checkpoint 에 기록되지 않는다')
    assert.equal(wd.report().sinceForwardMs, 0, '전진 시각은 갱신되지 않았다(시작 시점 그대로)')
  }
})

test('[fail-safe] 문구가 바뀐 채로 계속 돌면 stall 축이 잡는다 — 감시가 사라지지 않는다', () => {
  const clock = makeClock()
  const wd = createJobWatchdog({ now: clock.now })
  // 바뀐 문구로 30초마다 신호가 온다. 비활성 축은 계속 갱신되므로 ①로는 절대 안 잡힌다.
  while (clock.now() - wd.startedAtMs < STALL_MS) {
    clock.advance(30_000)
    wd.observe({ percent: 62, message: CHANGED_FORMATS[0] })
    assert.notEqual(wd.evaluate(), 'inactivity-timeout', '신호는 계속 오고 있다')
  }
  assert.equal(wd.evaluate(), 'no-forward-progress', '전진을 못 읽으면 보수적으로 잡는다')
})

test('[fail-safe] 문구 해석 실패는 판정을 절대 느슨하게 만들지 않는다(항상 forward 이하)', () => {
  // 같은 시나리오를 '현행 문구'와 '바뀐 문구'로 돌렸을 때, 바뀐 쪽이 더 오래 살아남으면 안 된다.
  const runUntilBreach = (msg: (i: number) => string): number => {
    const clock = makeClock()
    const wd = createJobWatchdog({ now: clock.now })
    for (let i = 0; i < 1000; i++) {
      clock.advance(30_000)
      wd.observe({ percent: 50, message: msg(i) })
      if (wd.evaluate() !== 'ok') return clock.now() - wd.startedAtMs
    }
    return Number.POSITIVE_INFINITY
  }
  const withCurrentFormat = runUntilBreach((i) => chunkMsg(i, 2000, 0, 1, '완료'))
  const withChangedFormat = runUntilBreach(() => CHANGED_FORMATS[0])
  assert.ok(withChangedFormat <= withCurrentFormat,
    `바뀐 문구(${withChangedFormat}ms)가 현행(${withCurrentFormat}ms)보다 오래 살면 안 된다`)
  assert.ok(Number.isFinite(withChangedFormat), '바뀐 문구도 반드시 언젠가 잡힌다')
})

// ═══════════════════════════════════════════════════════════════════════════
// H. 타이머 수명 — 취소·앱 종료 후 아무 것도 남지 않는다
// ═══════════════════════════════════════════════════════════════════════════

/** setInterval/clearInterval 시임 — 실제 타이머 없이 해제 여부를 관측한다. */
function makeFakeIntervals() {
  const handles: any[] = []
  const deps = {
    setInterval: (fn: () => void, ms: number) => {
      const h: any = { fn, ms, cleared: 0, unrefed: false }
      h.unref = () => { h.unrefed = true; return h }
      handles.push(h)
      return h
    },
    clearInterval: (h: any) => { if (h) h.cleared++ }
  }
  const tickAll = () => { for (const h of handles) if (h.cleared === 0) h.fn() }
  return { deps, handles, tickAll }
}

function startWatch(clock: { now: () => number }, iv: ReturnType<typeof makeFakeIntervals>,
                    isRunning: () => boolean = () => true) {
  const wd = createJobWatchdog({ now: clock.now })
  const breaches: string[] = []
  const h = startJobWatch({
    watchdog: wd, intervalMs: 15_000, isRunning,
    onBreach: (v) => breaches.push(v), deps: iv.deps as any
  })
  return { wd, h, breaches }
}

test('타이머는 unref 된다 — 이 감시가 앱 종료를 막지 않는다', () => {
  const iv = makeFakeIntervals()
  startWatch(makeClock(), iv)
  assert.equal(iv.handles.length, 1)
  assert.equal(iv.handles[0].unrefed, true)
})

test('stop() 은 타이머를 정확히 한 번 해제하고 멱등이다', () => {
  const iv = makeFakeIntervals()
  const { h } = startWatch(makeClock(), iv)
  assert.equal(h.stop(), true)
  assert.equal(h.stopped, true)
  assert.equal(h.stop(), false, '두 번째 stop 은 아무 일도 하지 않는다')
  assert.equal(iv.handles[0].cleared, 1, 'clearInterval 은 정확히 1회')
})

test('취소 경로: stop() 이후에는 예산을 한참 넘겨도 판정이 일어나지 않는다', () => {
  const clock = makeClock()
  const iv = makeFakeIntervals()
  const { wd, h, breaches } = startWatch(clock, iv)
  wd.observe({ percent: 30, message: chunkMsg(0, 2, 0, 1, '완료') })
  h.stop()                                  // ← audio.ipc 의 currentWatchdogClear / done 이 하는 일
  clock.advance(STALL_MS + jobBudgetMs(2))  // 모든 축을 한참 초과
  iv.tickAll()
  assert.equal(breaches.length, 0, '해제된 타이머는 절대 발화하지 않는다')
  assert.equal(h.tickCount, 0)
})

test('자식이 이미 끝났으면 점검하지 않는다 — 종료 중 오판 금지', () => {
  const clock = makeClock()
  const iv = makeFakeIntervals()
  let alive = true
  const { h, breaches } = startWatch(clock, iv, () => alive)
  alive = false
  clock.advance(STALL_MS + 60_000)
  iv.tickAll()
  assert.equal(h.tickCount, 0, 'isRunning=false 면 판정 자체를 건너뛴다')
  assert.equal(breaches.length, 0)
  assert.equal(h.stopped, false, '아직 stop 은 done 핸들러의 몫이다')
})

test('비활성 축(①)은 이 타이머가 다루지 않는다 — 중복 판정 금지', () => {
  const clock = makeClock()
  const iv = makeFakeIntervals()
  const { wd, h, breaches } = startWatch(clock, iv)
  clock.advance(INACTIVITY_MS + 1_000)
  assert.equal(wd.evaluate(), 'inactivity-timeout')
  iv.tickAll()
  assert.equal(breaches.length, 0, '①은 audio.ipc 의 resetWatchdog 타이머가 담당한다')
  assert.equal(h.stopped, false)
})

test('축이 터지면 스스로 멈춘다 — onBreach 는 정확히 1회, 이후 tick 은 무해', () => {
  const clock = makeClock()
  const iv = makeFakeIntervals()
  const { wd, h, breaches } = startWatch(clock, iv)
  // liveness 만 주며 stall 을 넘긴다(비활성 축에 먼저 걸리지 않게).
  while (clock.now() - wd.startedAtMs <= STALL_MS) {
    clock.advance(30_000)
    wd.observe({ percent: 24, message: loadMsg(1) })
    iv.tickAll()
  }
  assert.deepEqual(breaches, ['no-forward-progress'])
  assert.equal(h.stopped, true, '발화 후 스스로 해제된다')
  assert.equal(iv.handles[0].cleared, 1)
  clock.advance(600_000)
  iv.tickAll()
  assert.equal(breaches.length, 1, '이미 멈춘 타이머는 다시 발화하지 않는다')
})

test('onBreach 가 예외를 던져도 타이머는 이미 해제돼 있다(누수 없음)', () => {
  const clock = makeClock()
  const iv = makeFakeIntervals()
  const wd = createJobWatchdog({ now: clock.now })
  const h = startJobWatch({
    watchdog: wd, intervalMs: 15_000, isRunning: () => true,
    onBreach: () => { throw new Error('소비자 폭발 모의') }, deps: iv.deps as any
  })
  // liveness 로 비활성 축을 계속 갱신하면서 stall 직전까지 간다(①에 먼저 걸리지 않게).
  while (clock.now() - wd.startedAtMs <= STALL_MS) {
    clock.advance(30_000)
    wd.observe({ percent: 24, message: loadMsg(1) })
  }
  assert.equal(wd.evaluate(), 'no-forward-progress', '터질 축은 stall 이다')
  assert.throws(() => iv.tickAll(), /소비자 폭발 모의/)
  assert.equal(h.stopped, true, 'onBreach 전에 이미 stop 됐다')
  assert.equal(iv.handles[0].cleared, 1)
})

// ═══════════════════════════════════════════════════════════════════════════
// I. 배선 계약 — audio.ipc.ts 의 호출 순서를 그대로 미러링해 고정한다
// ═══════════════════════════════════════════════════════════════════════════
//
// audio.ipc.ts 는 electron 을 import 하므로 node --test 가 로드하지 못한다(유닛 테스트 불가).
// 그래서 '그 파일이 이 원시들을 어떤 순서로 부르는가'를 여기서 미러링해 고정한다. 배선을 바꾸면
// 이 테스트가 먼저 깨지도록 하는 것이 목적이다.

/** audio.ipc.ts 의 tts 실행 한 건을 원시 호출 순서만 그대로 옮긴 것. */
function mirrorRun(clock: { now: () => number }, iv: ReturnType<typeof makeFakeIntervals>) {
  const sent: { channel: string; data: unknown }[] = []
  const runnerState = { running: true }
  const gate = createStagingGate<unknown>(
    (d) => sent.push({ channel: 'audio:result', data: d }), createTerminalGate)
  const wd = createJobWatchdog({ now: clock.now })
  const tick = startJobWatch({
    watchdog: wd, intervalMs: 15_000, isRunning: () => runnerState.running,
    deps: iv.deps as any,
    onBreach: (v) => {
      gate.abandon()                                   // ← 터미널 확정 → 늦은 결과 봉쇄
      sent.push({ channel: 'audio:error', data: v })
    }
  })
  return {
    sent, gate, wd, tick, runnerState,
    onProgress: (p: any) => wd.observe(p),
    onResult: (d: unknown) => gate.offerFinal(d),      // 아직 공개 안 됨
    onCancel: () => { tick.stop(); gate.abandon() },   // ← currentWatchdogClear
    onDone: (opts: { swept: boolean }) => {            // ← done 정상 경로
      runnerState.running = false
      tick.stop()
      if (!opts.swept) return                          // staging 실패 → 공개 금지
      gate.markStagingComplete()
    }
  }
}

test('[배선] 정상 경로: 스윕(staging) 확인 후에만 결과가 공개된다', () => {
  const clock = makeClock()
  const iv = makeFakeIntervals()
  const r = mirrorRun(clock, iv)
  r.onProgress({ percent: 30, message: chunkMsg(0, 1, 0, 1, '완료') })
  r.onResult({ tracks: ['synthesized.wav'] })
  assert.equal(r.sent.length, 0, 'done 전에는 아무것도 나가지 않는다')
  r.onDone({ swept: true })
  assert.deepEqual(r.sent, [{ channel: 'audio:result', data: { tracks: ['synthesized.wav'] } }])
  assert.equal(iv.handles[0].cleared, 1, '타이머도 함께 정리된다')
})

test('[배선] staging 이 확인되지 않으면 결과는 공개되지 않는다', () => {
  const clock = makeClock()
  const r = mirrorRun(clock, makeFakeIntervals())
  r.onResult({ tracks: ['synthesized.wav'] })
  r.onDone({ swept: false })
  assert.equal(r.sent.length, 0, '중간 산출물 정리가 확인되지 않았다 → final 비공개')
  assert.equal(r.gate.hasPending, true)
})

test('[배선] watchdog 이 먼저 마감한 뒤 늦게 도착한 결과는 공개되지 않는다(이번에 닫은 구멍)', () => {
  const clock = makeClock()
  const iv = makeFakeIntervals()
  const r = mirrorRun(clock, iv)
  // 살아있지만 아무것도 못 만드는 상태로 stall 초과 → 터미널 확정.
  while (clock.now() - r.wd.startedAtMs <= STALL_MS) {
    clock.advance(30_000)
    r.onProgress({ percent: 24, message: loadMsg(1) })
    iv.tickAll()
  }
  assert.deepEqual(r.sent.map((s) => s.channel), ['audio:error'])
  // 그 뒤에 Python 이 뒤늦게 result 를 뱉고 done 이 정상 경로로 들어온다.
  r.onResult({ tracks: ['synthesized.wav'] })
  r.onDone({ swept: true })
  assert.deepEqual(r.sent.map((s) => s.channel), ['audio:error'],
    '시간 초과로 마감된 실행이 결과를 공개하면 안 된다')
  assert.equal(r.gate.outcome, 'abandoned')
})

test('[배선] 취소 경로: 타이머 해제 + 결과 봉쇄가 함께 일어난다', () => {
  const clock = makeClock()
  const iv = makeFakeIntervals()
  const r = mirrorRun(clock, iv)
  r.onResult({ tracks: ['synthesized.wav'] })
  ...
  assert.equal(iv.handles[0].cleared, 1)
  r.onDone({ swept: true })
  assert.equal(r.sent.length, 0, '취소된 실행의 결과는 공개되지 않는다')
  clock.advance(STALL_MS * 2)
  iv.tickAll()
  assert.equal(r.sent.length, 0, '취소 후 타이머 발화도 없다')
})
