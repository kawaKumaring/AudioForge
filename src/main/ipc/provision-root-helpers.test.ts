...
test('public status는 전체 경로를 포함하지 않고 label/token/opaque fingerprint만 반환', () => {
  ...
  const status = publicRootStatus('secret', record)
  assert.equal(status.configured, true)
  assert.equal(status.displayLabel, '사용자 선택 관리형 위치')
  assert.ok(!JSON.stringify(status).includes('D:\\secret'))
  assert.match(status.approvalContext!, /^[0-9a-f]{64}$/)
})

test('root 미선택은 명시적인 비준비 상태', () => {
  assert.deepEqual(publicRootStatus(null, null), {
    configured: false,
...
test('approval fingerprint는 profile/plan/root/volume 네 축 모두에 결합', () => {
  const base = { profile: 'minimal-qwen', planFingerprint: 'p1', approvalContext: 'c1' }
  ...
  assert.notEqual(a, approvalFingerprint({ ...base, approvalContext: 'c2' }))
