...
//    · 비활성 버튼은 회색 처리로 끝내지 않고 사유 문장을 함께 렌더한다.
//    · 표준 문구 '내용'은 렌더하지 않는다(계약 6) — 버전 번호만 표시.
...
export interface EmotionSamplerPanelProps {
  ...
  /** 감정 하나를 미리듣기 목록에 추가. 미주입이면 추가 UI 를 숨긴다. */
  onAddEmotion?: (emotionId: string) => void
  /** 감정 하나를 목록에서 제외(샘플 파일 삭제는 onDelete 가 담당). */
      ...
      {open && (
        <div id={BODY_ID} style={{ display: 'flex', flexDirection: 'column', gap: 8, minWidth: 0 }}>
          ...
          <ul style={{
            ...
            {views.length === 0 && (
              <li style={{ fontSize: 11, color: 'var(--text-muted)', overflowWrap: 'anywhere' }}>
