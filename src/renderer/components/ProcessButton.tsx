...
export default function ProcessButton() {
  ...
  const handleProcess = async () => {
    ...
    try {
      // 확정한 클립이 사라졌으면 여기서 다시 만들고 **그대로 이어서** 합성한다(사용자에게 되묻지 않는다).
      // 실패한 슬롯은 막지 않는다 — 기존 오류 경로가 사유를 그대로 말한다.
      const rec = await recoverClips((msg) => useAppStore.getState().setProgress(2, msg))
      if (rec.rebuilt > 0) console.log('[renderer][synthesize] 사라진 참조 클립 재생성', rec)
      if (rec.failed.length > 0) console.warn('[renderer][synthesize] 재생성 실패', rec.failed)
      ...
      const r = await window.api.audio.process(fileInfo.path, mode, { trimSilence, silenceGap, transcribe, translate, exportSrt, outputFormat, whisperModel, asrEngine, whisperLang, translateModel, demucsModel, nSpeakers, splitMarkers, splitLabels, ttsText, ttsSpeed, ttsSilenceGap, ttsPitch, ttsEmotionRefs: emotionRefsToSend, ttsEmotionRefSources: emotionSources, ttsEmotionRefRegions: emotionRegions,
