...
if (WITH_APP_UI) {
  ...
  // 파일을 열 때 되살아난 이전 작업을 알리고 새로 시작할 수 있는가(조용한 되살리기 금지).
  run('실제 앱 · 이전 작업 되살리기 알림(GPU 없음)', 'node',
    [path.join('test', 'e2e', 'work-draft-restore-notice.e2e.mjs')])
  ...
  // ★2026-09-24 2차 감사: 이 검사는 **이미 있었는데 게이트에 없어서**
  //   '수정본으로 다시 만들기' 가 사흘간 100% 실패하는 것을 아무도 몰랐다
  //   (정리 코드가 모양이 같은 앞 함수에 잘못 붙어 NameError).
  //   있는 검사를 게이트에 안 걸어 두면 없는 것과 같다.
  run('실제 앱 · 대화 구간 수정본 다시 만들기(GPU 없음)', 'node',
    [path.join('test', 'e2e', 'dialogue-edit.e2e.mjs')])
  ...
  for (const n of ['복원 중 선택 변경', '재생 음량 유지', '목소리 준비 표시 일치', '일반·고급 목소리 격리',
    ...
    '생성 카드 화면', '작업 흐름', '생성 카드 연결', '생성 카드 본체 통과']) {
    skip(`실제 앱 · ${n}`, '같은 이유')
  }
}
if (WITH_APP) {
